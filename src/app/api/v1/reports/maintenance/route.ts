  ...
  try {
    ...
    // and convert ramCantidad to a number ONLY if it's defined and not empty
     const prismaData = {
          ...
          marca: restData.marca ?? null,  // Use null coalescing operator
          modelo: restData.modelo ?? null,
          sistemaOp: restData.sistemaOp ?? null,
          procesador: restData.procesador ?? null,
          ram: restData.ram ?? null,
          ramCantidad: restData.ramCantidad ? Number(restData.ramCantidad) : null, // Keep as string
          diagnostico: restData.diagnostico,
          falla: restData.falla ?? null,
          causa: restData.causa ?? null,
    ...
    return NextResponse.json({ error: 'Error del servidor al crear el reporte' }, { status: 500 });
  }
}