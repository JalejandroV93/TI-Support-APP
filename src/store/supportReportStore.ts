...
interface SupportReportState {
  ...
  fetchReports: (page: number, pageSize: number) => Promise<void>; // Add fetchReports
  resetState: () => void; // Add resetState
...
export const useSupportReportStore = create<SupportReportState>((set, get) => ({
  ...
  createReport: async (reportData) => {
    try {
      ...
      if (res.ok) {
        const newReport: SupportReport = await res.json();
        get().addReportToList(newReport); // Add the new report optimistically
        return true;
      } else {
  ...
  updateReport: async (id, reportData) => {
    try {
      ...
      if (res.ok) {
        const updatedReport: SupportReport = await res.json();
          get().updateReportInList(id, updatedReport); // Optimistically update
  ...
  fetchReports: async (page, pageSize) => {
    ...
    addReportToList: (report: SupportReport) => {
      set((state) => ({ reports: [report, ...state.reports], totalCount: state.totalCount+1 })); // Add new report to the beginning of the list
    },
